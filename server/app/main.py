...
async def websocket_endpoint(websocket: WebSocket, game_id: str, player_id: str):
    ...
    players = redis_client.lrange(f"game:{game_id}:players", 0, -1)
    if player_id not in players: # type: ignore
        raise WebSocketException(code=status.WS_1008_POLICY_VIOLATION)
    await websocket.accept()
    logging.info("Websocket connection accepted.")
    try:
        game_data = json.loads(redis_client.get(f"game:{game_id}")) # type: ignore
        if game_data["status"] == "waiting":
            nplayers = redis_client.llen(f"game:{game_id}:players")
        ...
        redis_client.lrem(f"game:{game_id}:players", -1, player_id)
...
async def create_game(data: GameData):
    ...
    try:
        ...
        redis_client.rpush(f"game:{game_id}:players", json.dumps(player_data))
